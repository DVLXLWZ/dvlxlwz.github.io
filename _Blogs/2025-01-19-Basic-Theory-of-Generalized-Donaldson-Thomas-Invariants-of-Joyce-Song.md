...
>
> - (iii) Given $[(\mathfrak{R},\rho)]$ as above and a 1-isomorphism $\mathfrak{R}\cong[X/G]$ for $X$ a quasiprojective $\mathbb{K}$-variety and $G$ a very special algebraic $\mathbb{K}$-group acting on $X$ with maximal torus $T^{G}$, we have \[[(\mathfrak{R},\rho)]=\sum\_{Q\in\mathcal{Q}(G,T^{G})}F(G,T^{G},Q)\bigl{[}([X/Q], \rho\circ t^{\mathbb{Q}})\bigr{]},\] (2.10) where $t^{\mathbb{Q}}:[X/Q]\rightarrow\mathfrak{R}\cong[X/G]$ is the natural projection 1-morphism.
>
Here $F(G,T^{G},Q)\in\mathbb{Q}$ are a system of rational coefficients with a complicated definition in [50, SS6.2], which we will not repeat. In [50, SS6.2] we derive an inductive formula for computing them when $G=\mathrm{GL}(k,\mathbb{K})$.
>
...
>
Define _multiplication '._', _pushforwards_ $\phi\_{\*}$, _pullbacks_ $\phi^{\*}$, and _tensor products_ $\otimes$ on the spaces $\widehat{\mathrm{SF}},\widehat{\mathrm{SF}}(\*,\chi,\mathbb{Q})$ as in Definition 2.7, and _projections_ $\Pi\_{n}^{vi}$ as in section 2.3. The important point is that (2.6)-(2.9) are compatible with the relations defining $\widehat{\mathrm{SF}},\widehat{\mathrm{SF}}(\*,\chi,\mathbb{Q})$, or they would not be well-defined.
