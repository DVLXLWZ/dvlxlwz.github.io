...
\]
>
> Define $\underline{\mathrm{SF}}(\mathfrak{F})$ to be the $\mathbb{Q}$-vector space generated by $[(\mathfrak{R}, \rho)]$ with $\rho$ representable, with the same relations above. Then $\underline{\mathrm{SF}}(\mathfrak{F}) \subseteq \underline{\mathrm{SF}}(\mathfrak{F})$.
